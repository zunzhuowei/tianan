...
<body>
<table class="t1" id="selfWanChengQingKuang">
    <tbody>
    <tr class="firstRow">
    ...
    <tr id="${tap.p04.p0400 }" ><!-- onclick="changeRow(this)"  -->
        ...
        <td width="60" valign="middle" align="center" colspan="1"
            rowspan="1" style="word-break: break-all;" id="selfAdminRank">
			<#if tap.target != null>
        ...
        <td width="125" valign="middle" align="center" colspan="1"
            rowspan="1" style="word-break: break-all;" id="wanChengQingKuang">
			<#if tap.target != null>
                <textarea name="P04Z9" onchange="autoSaveSeft(this)" style="width: 100%;height: 60px">${tap.p04.p04Z9 }</textarea>
			<#else>
                <span></span>
			</#if>
        </td>
        <td width="60" valign="middle" align="center" colspan="1"
            rowspan="1" style="word-break: break-all;" id="ziPingFenShu">
            <input type="text" value="${tap.evaluationSelfOther.selfScore }" onchange="autoSaveEvaValue1(this)" name="sScore" style="width: 100%"/>
        </td>
        <td width="60" valign="middle" align="center" colspan="1"
            rowspan="1" style="word-break: break-all;">
    ...
    <tr id="${sape.secondP04.p0400 }"><!--  onclick="secondChangeRow(this)" -->
        ...
        <td valign="middle" align="center" colspan="1" rowspan="1"
            style="word-break: break-all;" id="selfAdminRank">
            <span>${sape.secondP04.p0415 }</span>
        </td>
        <td valign="middle" align="center" colspan="1" rowspan="1"
            style="word-break: break-all;" id="wanChengQingKuang">
            <textarea name="P04Z9" onchange="secondSaveSeft(this)" style="width: 100%;height: 60px">${sape.secondP04.p04Z9 }</textarea>
        </td>
        <td valign="middle" align="center" colspan="1" rowspan="1"
            style="word-break: break-all;" id="ziPingFenShu">
            <input type="text" value="${sape.evaluationSelfOther.selfScore }" onchange="autoSaveEvaValue2(this)" name="sScore" style="width: 100%"/>
        </td>
        <td valign="middle" align="center" colspan="1" rowspan="1"
            style="word-break: break-all;">
...
<table class="t1" style="display:none">
    ...
    <tr id="${cdas.common2Desc.point_id }" onclick="getChangeScore(this)">
        ...
        <td valign="middle" align="center" colspan="1" rowspan="1"
			...
			<#else>
                <input type="text" onchange="saveCommonScore(this.value,this.name)" disabled="disabled" value="" name="Score" style="width: 100%"/>
			</#if>
        </td>
    </tr>
	</#list>
...
<script type="text/javascript">
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    //selfWanChengQingKuang
/*    function retunRank2(cardMain){//还有下拉列表为验证！id="selfAdminRank"
        var tableId = cardMain.document.getElementById("tb1");
        //alert(tableId);
        if(tableId == null){
            return ;
        }
        ...
        for(var i=4;i<tableId.rows.length;i++){
            ...
            for(var j = 0;j<cellList.length;j++){
                ...
                if(rank=="rank"){
                    if(parseFloat(tableId.rows[i].cells[j].getElementsByTagName("input")[0].value) == 0){
                        return 0 ;
                    }
                    score1 = score1 + parseFloat(tableId.rows[i].cells[j].getElementsByTagName("input")[0].value);
                    //alert(rank);
                }
                if(rank=="rank_text"){
                    score2 = score2 + parseFloat(tableId.rows[i].cells[j].innerText);
                    //alert(tableId.rows[i].cells[j].innerText);
                }
            }
        }
    ...
    function refashRecord(e) {//实时显示自评分数
        ...
        if(tableId != undefined){
            for(var i=4;i<tableId.rows.length;i++){
                ...
                for(var j=0;j<cellList.length;j++){//scoreEva
                    if (cellList[j].id == "selfAdminRank") {
                        var rankText = parseFloat(cellList[j].innerText);
                        if(cellList[j+2].id == "ziPingFenShu"){
                            var rank = cellList[j+2].getElementsByTagName("input")[0].value;
                            if (rank>rankText || rank<0) {
                                alert("自评的分数不能超过权重分值、也不能小于0；即：0≤自评分数≤权重分值");
                                return;
                            }
                            if(rank != ""){
        ...
        document.getElementById("nowRecord").innerHTML = "当前自评分数：("+flag+")分";
        return flag;
    }

    function IsNumeric1(input){
        //var RE = /^[0-9]{1,26}\.?[0-9]{0,2}$/;
        var RE = /^[1]?[0-9]{1,2}\.?[0-9]{0}$/;
        return (RE.test(input));
    }
